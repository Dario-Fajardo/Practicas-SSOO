...
std::optional<sockaddr_in> Socket::make_ip_address(const std::optional<std::string>& ip_address_string, uint16_t port) const {
  ...
  if (ip_address_string.has_value()) {
    ...
    inet_aton(ip_address_string_copy.c_str(), &ip_address.sin_addr);
  } else {
...
/**
 * Función de la clase Socket que permite recibir mensajes de una dirección IP
 * 
 * @param transmitter Estructura del tipo sockaddr_in en la que se guardará la dirección del emisor
 * 
...
std::string Socket::ip_address_to_string(const sockaddr_in& address) const {
  std::string string_address = inet_ntoa(address.sin_addr);
  if (ntohs(address.sin_port) != 0) {
    string_address += ":";
    string_address += std::to_string(ntohs(address.sin_port));
  }
